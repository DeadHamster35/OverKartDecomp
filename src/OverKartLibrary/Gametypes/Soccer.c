...
const CTFSpawn *SoccerGameSpawns[4] = {&SoccerBigDonutSpawns, &SoccerBlockFortSpawns, &SoccerDoubleDeckerSpawns, &SoccerSkyscraperSpawns};
...
{    
    if (HotSwapID > 0)
    {
        ...
        {
            int ThisObj;
        for (ThisObj = 0; ThisObj < 64; ThisObj++)
        {
        ...
        {
            int ThisFlag;
        for (ThisFlag = 0; ThisFlag < g_playerCount; ThisFlag++)
        {    
    ...
    {
        int ThisFlag;
    for (ThisFlag = g_playerCount; ThisFlag < 4; ThisFlag++)
    {
...
void CaptureBalls(void)
{
    ...
    {
        int ThisFlag;
    for (ThisFlag = 0; ThisFlag < FlagCount; ThisFlag++)
    {
    ...
    for (ThisPlayer = 0; ThisPlayer < g_playerCount; ThisPlayer++)
    {
        if (Objectives[ThisPlayer].FlagHeld != 0)
        {
            {
                int ThisBase;
            for (ThisBase = 0; ThisBase < FlagCount; ThisBase++)
            {
