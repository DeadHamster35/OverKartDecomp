...
void GetLinear(OKSkeleton *Bone, float Scale, int Frame)
{
    ...
    else
    {
        {
            int ThisKey;
        for (ThisKey = 0; ThisKey < Bone->PositionCount; ThisKey++)
        {
    ...
    RotationData = (PolyKeyFrame *)(GetRealAddress(Bone->RotationData | 0x0A000000));
    ...
    else
    {
        {
            int ThisKey;
        for (ThisKey = 0; ThisKey < Bone->RotationCount; ThisKey++)
        {
    ...
    /*
        ...
        else
        {
            {
                int ThisKey;
            for (ThisKey = 0; ThisKey < AnimeData->ScalingCount; ThisKey++)
            {
...
void DrawSkeleton(OKSkeleton *BoneData, int CurrentFrame)
{
    ...
    {
        int CurrentNode;
    for (CurrentNode = 0; CurrentNode < BoneData->NodeCount; CurrentNode++)
    {
        ...
        MeshOffsets = (uint *)(GetRealAddress(NodeArray[CurrentNode].MeshOffset | 0x0A000000));
        

        {
            uint CurrentMesh;
        for (CurrentMesh = 0; CurrentMesh < NodeArray[CurrentNode].MeshCount; CurrentMesh++)
        {
    ...
    {
        int ThisChild;
    for (ThisChild = 0; ThisChild < BoneData->ChildCount; ThisChild++)
    {
        GlobalAddressA += sizeof(OKSkeleton);
        Skeletor = (OKSkeleton *)GlobalAddressA;
