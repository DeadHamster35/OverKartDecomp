...
These are passed to DrawLocalSkeletonShape, along with what appears to be an index and a timer.
DrawLocalSkeletonShape(unk70, unk74, index, timer)
...
They are handled in draw_skeleton_entry.
...
They seem to be used in hmsdraw_joint to set some values in Vec3f which is then used to set some values in a
...
They're used in draw_skeleton_entry and hmsdraw_joint, and they're always used in triples
The `timer` argument to DrawLocalSkeletonShape is compared to the `animation_length` entry.
...
The first triplet is used in draw_skeleton_entry to access the Type 2 array and the values accessed is placed into
shapepos_offset. shapepos_offset is a Vec3s is then used to set the values of some Vec3f in
hmsdraw_joint. All further triplets are used in hmsdraw_joint to collect Type 2 values to another
...
The chosen values are then used to access the Type 2 array and the value accessed is placed into shapepos_offset.
shapepos_offset is a Vec3s that is used to set values in the some Vec3f that goes on to be used to modify the same
...
s16 check_skelanime_end(Animation**, s16);
void hmsdraw_joint(Armature*, s16*, AnimationLimbVector, s32);
void draw_skeleton_entry(Armature*, Animation*, s16);
s16 DrawLocalSkeletonShape(Armature*, Animation**, s16, s16);
