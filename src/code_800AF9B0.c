...
void alloc_flagobj(void) {
    D_8018EDB8 = (void*) AllocateMemory(480 * sizeof(Vtx));
    D_8018EDBC = (void*) AllocateMemory(480 * sizeof(Vtx));
}

// could be a normal vertex, not a color...
void draw_1_square(Vtx* arg0, s32 arg1, s32 arg2, s32 arg3, s16 arg4, s16 arg5, s32 arg6, s32 arg7) {
    s32 r, g, b;
    s32 i;
...
void calc_nvector(Vtx* vtx, s32 a, s32 b, s32 c, Vec3s out) {
    s32 varA0;
    s32 varA2;
    s32 varA4;
    s32 varB0;
...
void set_gshade(Vtx* arg0, Vec3s* arg1, s32 arg2, s32 arg3) {
    s32 idx1;
    s32 idx2;
    s32 i;
    Vtx* vtx;
...
void set_vtx_nvector(Vtx* arg0) {
    UNUSED u32 pad88[26];
    s32 i, j;
    s16 sp40[12][3];

    for (i = 0, j = 0; i < ARRAY_COUNT(sp40); i++, j += 4) {
        calc_nvector(&arg0[j], 1, 2, 0, sp40[i]);
    }

    for (i = 0, j = 0; i < ARRAY_COUNT(sp40); i++, j += 4) {
        set_gshade(&arg0[j], sp40, i, 11);
    }
}
...
void draw_flag(void) {
    ...
    for (i = 0; i < 10; i++) {
        ...
        res1 = ((sinT(D_8018EDB0 - (j * D_8018EDB2)) * 84.0f) * j) * 0.18f;
        res2 = ((sinT(D_8018EDB0 - ((j + 1) * D_8018EDB2)) * 84.0f) * (j + 1)) * 0.18f;
        draw_1_square(&(&vtxs[j * idx])[i * 48], j, i, 84, res1, res2, k, 84);
        }
    }

    set_vtx_nvector(vtxs);
