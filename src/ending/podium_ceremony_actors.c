...
void balloon_update(CeremonyActor* actor) {
    draw_balloon(actor->pos, 1.0f, actor->unk2E, actor->unk2C);
    actor->pos[1] += 0.8f;
    actor->unk2E = sinT(actor->unk30) * actor->unk34;
...
void func_80280A28(Vec3f arg0, Vec3s arg1, f32 arg2) {
    ...
    Affine2Mtx(&gDynamicP->mtxEffect[effectcount], mtx);
...
void balloons_and_fireworks_init(void) {
    resultstat.total_timer = 0;
    sPodiumActorList = (CeremonyActor*) AllocateMemory(sizeof(CeremonyActor) * 200);
    bzero(sPodiumActorList, (sizeof(CeremonyActor) * 200));
    new_actor(&initDummy);
}

void func_80280FFC(void) {
    resultstat.carmove = 1;
}
...
void spawn_timer(void) {
    ...
    guLookAtF(D_80287500, camera->camera_pos[0], camera->camera_pos[1], camera->camera_pos[2], camera->lookat_pos[0], camera->lookat_pos[1],
              camera->lookat_pos[2], camera->up_vector[0], camera->up_vector[1], camera->up_vector[2]);
    if (resultstat.rank < 3) {
        if (resultstat.total_timer < 300) {
            lookAtY = camera->lookat_pos[1];
            spawn_firework_cone(-0xE0E, (s32) (((lookAtY - camera->camera_pos[1]) * 1.5f) + lookAtY), -0x258);
        }
        if (resultstat.total_timer == 120) {
            spawn_balloons(-0xC6C, (s32) ((f32) D_802874B0[10] + 210.0f), -0x1EF);
        }
    } else if (resultstat.total_timer == 2) {
        spawn_balloons(-0xC6C, (s32) ((f32) D_802874B0[10] + 210.0f), -0x1EF);
    }

    resultstat.total_timer += 1;
}
...
void ResultsSequence(void) {
    gMatrixCount = 0;
    D_802874FC = 0;
    result_camera_control();
    kartpos_control_full();
    ken_startegy();
    KWVideoFramesYori();
    KWVideoFramesYori();
    kwgameframesyori();
    if (resultstat.carmove != 0) {
        oga_hyousyou_move();
        EtcEnemyDrive();
    }
    ResultsCheck();
