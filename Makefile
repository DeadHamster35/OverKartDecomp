...
# OverKartLibrary extra code will not match the stock US ROM.
COMPARE ?= 0
...
# OverKart Library C (copied from sibling Library/). Do not flatten.
# CustomFuncs/ is copied but not on the first compile list.
OVERKART_SRC_DIRS := \
  src/OverKartLibrary \
  src/OverKartLibrary/CustomCourses \
  src/OverKartLibrary/CustomObjects \
  src/OverKartLibrary/CustomParticles \
  src/OverKartLibrary/MusicSound \
  src/OverKartLibrary/Player \
  src/OverKartLibrary/Gametypes \
...
# Fills end of rom (16MB; .overkart starts at ROM 0xC00000)
